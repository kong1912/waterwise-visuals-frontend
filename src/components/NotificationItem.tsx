...
export const NotificationItem = ({ notification }: { notification: Notification }) => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "moistureHigh":
        return <Droplet className="w-5 h-5 text-red-500" />; // Red for high moisture
      case "moistureLow":
        return <Droplet className="w-5 h-5 text-blue-500" />; // Blue for low moisture
      case "temperatureHigh":
        return <Activity className="w-5 h-5 text-red-500" />; // Red for high temperature
      case "brightnessLow":
        return <Bell className="w-5 h-5 text-blue-500" />; // Blue for low brightness
      case "motionDetected":
